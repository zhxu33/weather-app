...
function searchCity(find) {
    ...
    .then(data => {
        ...
        tempstatP.innerText = "L:"+Math.round(data['main']['temp_min'])+'° '+ "H:"+Math.round(data['main']['temp_max'])+'°';
        document.body.style.backgroundImage = "url('"+ "https://source.unsplash.com/1600x900/?"+cityText +"%20city')";
    });
}
...
function render() { //reset list of cities
    ...
    cityList.forEach(function (cityTitle) {
        ...
        fetch('https://api.openweathermap.org/data/2.5/weather?q='+cityTitle+"&units=imperial"+'&appid=b973b62226fb5128e50d6ab73540ec56')
        .then((response) => {
        if (!response.ok) {
            alert("No weather found.");
            throw new Error("No weather found.");
        ...
        .then(data => {
            element.getElementsByClassName("sample-temp")[0].innerText = Math.round(data['main']['temp'])+'°';
            element.getElementsByClassName("sample-weather")[0].innerText = data['weather'][0]['main'];
        });
